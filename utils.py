...
def load_data(directory, random_seed):
    ...
    return ID, IM, samples ,ID1, IM1     # 未知关联数量较多，选择和已知关联数目相同的未知关联组成样本


def build_graph(directory, random_seed):
    ID, IM, samples,ID1, IM1 = load_data(directory, random_seed)
    ...
    d_sim1 = torch.zeros(g.number_of_nodes(), ID1.shape[1])  # 创建一个张量来存储disease相i似性特征，初始值为0
    d_sim1[: ID1.shape[0], :] = torch.from_numpy(ID1.astype('float32'))  # 将从文件加载的dsease相似性特征存储在d_sim张量中
    g.ndata['d_sim1'] = d_sim1
    ...
    m_sim1 = torch.zeros(g.number_of_nodes(), IM1.shape[1])  # 创建一个张量来存储miRNA相似性特征，初始值为0
    m_sim1[ID1.shape[0]: ID1.shape[0] + IM1.shape[0], :] = torch.from_numpy(
        IM1.astype('float32'))  # 将从文件加载的miRNA相似性特征存储在m_sim张量中
    g.ndata['m_sim1'] = m_sim1  # 将m_sim存储在图的节点特征'data'中
    ...
    return g, sample_disease_vertices, sample_mirna_vertices, ID, IM, samples,ID1, IM1  # 返回构建的图以及其他相关变量
